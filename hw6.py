class VendingMachine(object):
    ...
    def vend(self):
        ...
        elif self.inventory > 0 and self.balance > self.price:
            phrase =  'Here is your {0} and ${1} change.'.format(self.product, self.balance - self.price)
        else:
            phrase = 'Here is your {0}.'.format(self.product)
        self.balance = 0
        self.inventory -= 1
        return phrase

    def deposit(self, amount):
        assert amount > 0, 'Deposited amount is not positive'
        if self.inventory == 0:
    ...
    def restock(self, amount):
        assert amount > 0, 'Restock amount is not positive'
...
class MissManners(object):
    ...
    def ask(self, string, *args):
        magic_word = 'please '
        if magic_word not in string:
            return 'You must learn to say please first.'
        function = string[len(magic_word):]
        if not hasattr(self.obj, function):
