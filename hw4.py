...
r1 = interval(1,1)
r2 = interval(2,1)
print(str_interval(par1(r1,r2)), ' != ', str_interval(par2(r1,r2)))

def multiple_references_explanation():
    return """The mulitple reference problem exists. The true
    value within a particular interval is fixed. Nested combinations
    that refer to the same interval, which is an error that results in
    intervals that are larger than they should between

    For example, in the case of i * i, where i is an interval from -1 to 1.
    No value within the interval, when squared will give a negative result.
    However, out mul_interval function will allow us to choose 1 from the first referenceto i and -1 from the second
    giving an erroneous lower bound of -1.

    Therefore, a program like par2 is better than par1 because it never combiens the same
